...
local Harvest = {
    ...
    Fish = game:GetService("Workspace").FishSpawns.Pond.Fish
}
...
local Window = Library:CreateWindow("Generic GUI")
...
function GetHarvestable(Handle, Crop)
    ...
    for _, A_1 in next, Harvest[Crop]:GetChildren() do
        local Middle = A_1:FindFirstChild("Middle", true) or A_1:FindFirstChild("Root")
...
Landlord:AddToggle({
    ...
    callback = function()
        while Library.flags.Rent do
            local Distance = Library.flags.Rent_Range
...
Farming:AddToggle({
    ...
    callback = function()
        ...
        while Library.flags.RiceWalk do
            local Sickle = GetTool("Sickle")
            if(not Sickle) then
              if(Player.Backpack:FindFirstChild("Sickle")) then
                Player.Character.Humanoid:EquipTool(Player.Backpack:FindFirstChild("Sickle"))
              end
              Sickle = Player.Backpack:FindFirstChild("Sickle") or GetTool("Sickle")
            end
            if(Sickle) then
              local Rice_To_Cut = false
              local Distance = 100
              for _, A_1 in next, Harvest["Rice"]:GetChildren() do
                  local Middle = A_1:FindFirstChild("Middle", true) or A_1:FindFirstChild("Root")
                  if Middle and A_1:FindFirstChild("Health") and A_1.Health.Value > 0 then
                      local Dist = (Sickle.Handle.Position - Middle.Position).magnitude
                      if Dist < Distance then
                          Distance = Dist
                          Rice_To_Cut = Middle
                      end
              ...
              if(Rice_To_Cut) then
                game.Players.LocalPlayer.Character.Humanoid:MoveTo(Rice_To_Cut.Position)
              end
            end
            task.wait()
        end
...
Farming:AddToggle({
    ...
    callback = function()
        -- Tree Chopping
        while Library.flags.Fish do
            local Axe = GetTool("Fishing Net")
            if Axe then
                local Closest = false
                local Distance = Library.flags.Farming_Range
                for _, A_1 in next, game:GetService("Workspace").FishSpawns.Pond.Fish:GetChildren() do
                    local Middle = A_1:FindFirstChild("Middle", true) or A_1:FindFirstChild("Root")
                    if Middle and A_1:FindFirstChild("Health") and A_1.Health.Value > 0 then
                        local Dist = (Axe.Handle.Position - Middle.Position).magnitude
                        if Dist < Distance then
                            Distance = Dist
                            Closest = Middle
                        end
                ...
                if Closest ~= false then
                    Axe:Activate()
                    Touch(Axe.Handle, Closest)
                    Touch(Axe.Handle, Closest.Parent.Body)
                end
            end
            task.wait(0.1)
        end
